...
统一的管理后台布局:左侧导航(概览/账号/商品与发货规则/订单/待处理/设置,待处理数带角标)+ 顶栏(页面标题/主题切换/退出),内容区卡片化;支持明暗双主题(记忆选择),窄窗口侧栏自动折叠。

- **概览**:在线账号、今日订单、今日已自动交付、待人工处理四张统计卡(点击下钻),恢复隔离横幅,账号速览(异常优先)与最近订单/待处理;页面可见时每 30 秒自动刷新,可手动刷新。
- **账号**:卡片视图(状态徽章、平台号脱敏、心跳时间);扫码接入弹窗含倒计时与超时重取;启停走确认流并显示"暂停中"过渡态。
- **商品与发货规则**:商品筛选(账号/已配置/关键字)与平台同步;规则编辑抽屉带实时校验、字数统计、内容预演(服务端)与**匹配预览**——由交付引擎判定"命中/未命中/冲突",界面不复现匹配逻辑。
- **订单**:组合筛选+分页(翻页保留条件);详情抽屉含三轴状态时间线、尝试记录、默认折叠的交付内容揭示,人工动作按当前状态启停并注明不可用原因(服务端 guard 仍是权威)。
- **待处理**:按类别分组的工作台;补发、终止等改变交付结果的操作原因必填、高危需勾选风险确认。
- **设置**:运行信息、会话安全、备份/恢复操作指引与后果说明。
